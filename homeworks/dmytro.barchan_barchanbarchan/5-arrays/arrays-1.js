...
const func = () => arr.reduce((accumulatorArr, currentValueArr) => {
    const str = currentValueArr.reduce((accumulatorStr, currentValueStr) => {
        const element = currentValueStr < 5 ? ' ' : '*';

        return accumulatorStr + element;
    }, '');

    return [...accumulatorArr, str];
}, []);
